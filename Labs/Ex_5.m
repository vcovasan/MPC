...
Px = [eye(2);-eye(2)];
qx = [Xmax;-Xmin];
%final state constraint matrices
%Pxf*x(k+N|k) <= qxf
Pxf = [eye(n);-eye(n)];
qxf = repmat([0;0],n,1);
% [Pc, qc, Sc] = constraint_mats(F,G,Pu,qu,Px,qx,Pxf,qxf); %with final constraint
[Pc, qc, Sc] = constraint_mats(F,G,Pu,qu,Px,qx,[],[]); %%without final constraints
...
nk = 20; %same as the horizon but doesn't have to be
